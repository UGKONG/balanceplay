...
export default function 이용권수정 ({ _data, getList, setIsEdit }) {
  const [useType, setUseType] = useState([]);
  const [data, setData] = useState({
    ID: _data?.ID, CATEGORY_ID: _data?.CATEGORY_ID,
    NAME: _data?.NAME, PLACE: useNumber(_data?.PLACE), USE_TYPE: _data?.USE_TYPE,
    USE_COUNT: String(_data?.USE_COUNT), USE_DAY: String(_data?.USE_DAY)
  });
  ...
  const submit = send => {
    useAxios.put('/voucher/' + send?.ID, { data: send }).then(({ data }) => {
      if (!data?.result) return useAlert.error('알림', data?.msg);
      useAlert.success('알림', '이용권이 수정되었습니다.');
      getList();
      setIsEdit(false);
    })
  }
  ...
  return (
    <Wrap>
      ...
      <Body>
        ...
        <Row>
          이용권 금액: 
          <Input value={data?.PLACE ? useNumber(data?.PLACE) : ''} style={{ minWidth: 110 }}
