...
const FamilyLi = ({ item, idx, setData }) => {
  ...
  const itemChanged = () => {
    setData(x => {
      let temp = [...x];
      temp[idx] = {
        type, birth, name, isTogether, description
      }
      return temp;
    });
  }
    
  useEffect(itemChanged, [type, birth, name, isTogether, description]);
...
export default function ({ id, title, allCount, next, setTotalData }) {
  ...
  const familyTypeList = useRef([
    ...
    { name: "누나", value: "olderOlderSister" },
    { name: "남동생", value: "youngerBrother" },
    { name: "언니", value: "olderSister" },
    { name: "오빠", value: "olderOlderBrother" },
  ...
  return (
    <>
      ...
      <RowList>
        {data.map((item, idx) => (
          <FamilyLi key={idx} item={item} idx={idx} setData={setData} />
        ))}
